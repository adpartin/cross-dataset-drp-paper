"""
Step 2: Compute G Matrices from Performance Scores
===================================================

This step generates cross-study analysis (CSA) G matrices from computed performance scores.
...
INPUT:
------
- Score CSV files from outputs/s1_scores/ directory (generated by Step 1)
- Files: all_models_scores.csv and <MODEL>_scores.csv for each model

OUTPUT:
-------
- Creates s2_G_matrices/ directory with CSA table files
...
- All matrices saved to outputs/s2_G_matrices/
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Step 2: Compute G matrices from performance scores.',
    ...
    parser.add_argument(
        ...
        type=str,
        help='Input directory containing scores from Step 1 (default: ./outputs/s1_scores)'
    )
    parser.add_argument(
        '--outdir',
        default='./outputs',
    ...
    outdir = Path(args.outdir) / 's2_G_matrices'
    ...
    logging.info(f"\n{'=' * 50}")
    logging.info("Step 2: Compute G Matrices from Performance Scores")
    logging.info("=" * 50)
    ...
    if not input_dir.exists():
        ...
        logging.error("Please run Step 1 (s1_compute_scores.py) first to generate scores.")
    ...
    if not required_file.exists():
        ...
        logging.error("Please run Step 1 (s1_compute_scores.py) first.")
    ...
    logging.info(f"Step 2 complete. G matrices saved to {outdir}")
